...
def main():
    ...
    display_note("These methid removes leading and trailing/from the beginning (left side)/from the end (right side) whitespace characters from the string.")
    ...
    display_note("All 3 methods remove characters, not substrings. Ex: 'strip('lo')' will remove all 'l' and 'o' characters from both ends of the string, not just the substring 'lo'.","WARNING")
    wstring = '  $$Hello World!!!'
...
"Stripped '!!!' from end of '" + wstring + "' string by 'rstrip('!')' method: " + wstring.rstrip('!'))
    ...
    print_small_sub_heading("The 'removeprefix(prefix)' and 'removesuffix(suffix)' methods are used to remove a specified prefix or suffix from the string.",True)
    display_note("If the prefix/suffix is not found or not pass, it returns the original string unchanged.")
    display_note("The 'removeprefix()' and 'removesuffix()' methods are available in Python 3.9 and later versions. They are not available in earlier versions of Python.","WARNING")
...
"Removing suffix 'world!' from '" + string1 + "' string by 'removesuffix()' method: " + string1.removesuffix('world!'))                  
    ...
    display_note("The 'center()/ljust()/rjust()' methods can also take a fill character as an optional argument. If not provided, it defaults to space.","NOTE")
