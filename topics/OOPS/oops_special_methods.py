...
def main(topic_number: int):
    print_heading("Special (Magic/Dunder) Methods in OOP", topic_number)
    imp_note_points("""
...
- Their names always start and end with double underscores (e.g., \_\_init\_\_, \_\_str\_\_).
...
`__str__` is for creating a user-friendly, readable output for end-users (used by `print()` and `str()`).
`__repr__` is for creating an unambiguous, developer-focused representation of the object, which should ideally be valid Python code to recreate the object (used by `repr()`).""")
...
`__eq__` implements the equality operator (`==`). `__ne__` implements the inequality operator (`!=`).""")
    display_note("""Default Behavior: If not defined, `__eq__` checks if two references point to the exact same object in memory (identity check, like the `is` operator). 
`__ne__` is the logical opposite.""", "example")
...
It's a great way to provide a default value or custom behavior for missing attributes.""")
...
`__len__` is called by `len()` and should return the object's length.
`__getitem__` is used for accessing items via a key or index, like `my_object[5]` or `my_object['name']`.""")
...
`__enter__` is called when entering the `with` block. It should return the resource to be managed.
`__exit__` is called when exiting the `with` block, even if an error occurred. It's used for cleanup.""")
