...
def main(topic_number: int):
    print_heading("Example of Functions", topic_number)
    imp_note_points('''IMPORTANT POINTS
- Functions in Python are defined using the ___def___ keyword, and they can take parameters and return values.
...
#--------------------------------------------------------#'''
    ...
    code3 = '''# function with default parameter
...
#--------------------------------------------------------------------#'''
    ...
    code4 = '''# function with variable length arguments
...
This improves readability and allows parameters to be passed in any order.""")
    code5 = '''# function with keyword arguments
...
This provides flexibility when passing dynamic named data to functions.""")
    code6 = '''# function with *kwarg
...
They are typically used for short, throwaway functions without formally defining them using `def`.""")
    code7 = '''# Lambda function
square = lambda x: x * x
...
These annotations do not enforce type checking at runtime— they're mainly used for documentation, IDE support, and static analysis tools.""")
    code8 = '''# function with annotations
...
The factorial of a number `n` is calculated as `n * (n-1) * (n-2) * ... * 1`.
Always include a base case (`n == 0 or 1`) to prevent infinite recursion.""")
    code9 = '''# Recursive function
...
Docstrings are used to document how a function should be used. They can be accessed at runtime using the `__doc__` attribute.""")
    code10 = '''# Function with docstring
